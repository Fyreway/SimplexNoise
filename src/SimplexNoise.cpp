...
// MODIFIED BY ISAAC YEE TO USE A SEED
static inline uint8_t hash(int32_t i, const std::array<uint8_t, 256> &perm) {
 ...
 * coordinates.
 */
float SimplexNoise::noise(float x, const std::array<uint8_t, 256> &perm) {
    ...
    t0 *= t0;
    n0 = t0 * t0 * grad(hash(i0, perm), x0);

    // Calculate the contribution from the second corner
    float t1 = 1.0f - x1 * x1;
    //  if(t1 < 0.0f) t1 = 0.0f; // not possible
    t1 *= t1;
    n1 = t1 * t1 * grad(hash(i1, perm), x1);
...
float SimplexNoise::noise(float x,
    ...
    const int gi0 = hash(i + hash(j, perm), perm);
    const int gi1 = hash(i + i1 + hash(j + j1, perm), perm);
    const int gi2 = hash(i + 1 + hash(j + 1, perm), perm);
...
float SimplexNoise::noise(float x,
    ...
    int gi0 = hash(i + hash(j + hash(k, perm), perm), perm);
    int gi1 = hash(i + i1 + hash(j + j1 + hash(k + k1, perm), perm), perm);
    int gi2 = hash(i + i2 + hash(j + j2 + hash(k + k2, perm), perm), perm);
    int gi3 = hash(i + 1 + hash(j + 1 + hash(k + 1, perm), perm), perm);
...
float SimplexNoise::fractal(size_t octaves,
    ...
    for (size_t i = 0; i < octaves; i++) {
        output += (amplitude * noise(x * frequency, perm));
        denom += amplitude;
...
float SimplexNoise::fractal(size_t octaves,
    ...
    for (size_t i = 0; i < octaves; i++) {
        output += (amplitude * noise(x * frequency, y * frequency, perm));
        denom += amplitude;
...
float SimplexNoise::fractal(size_t octaves,
    ...
    for (size_t i = 0; i < octaves; i++) {
        output += (amplitude
                   * noise(x * frequency, y * frequency, z * frequency, perm));
        denom += amplitude;
